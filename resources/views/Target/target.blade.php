        ...
        <table class="table bordered-table mb-0" id="dataTable" data-page-length='10'>
          ...
          <tbody>
              ...
              <tr>
                  ...
                  <td>
                      @if($target->technology && $target->technology->has_cms)
                          {{ $target->technology->cms_name }}
                          @if($target->technology->cms_version)
                              <span class="text-muted">(v{{ $target->technology->cms_version }})</span>
                          @endif
                      @else
                          N/A
                      @endif
                  </td>
                  <td>{{ $target->last_scan_date ? $target->last_scan_date->format('d M Y') : 'Never' }}</td>
                  ...
                  <td>
                      <a href="{{ route('targets.show', $target->id) }}" class="w-32-px h-32-px bg-primary-light text-primary-600 rounded-circle d-inline-flex align-items-center justify-content-center">
                          <iconify-icon icon="iconamoon:eye-light"></iconify-icon>
                      </a>
                      <a href="{{ route('targets.edit', $target->id) }}" class="w-32-px h-32-px bg-success-focus text-success-main rounded-circle d-inline-flex align-items-center justify-content-center">
                          <iconify-icon icon="lucide:edit"></iconify-icon>
                      </a>
                      <form action="{{ route('targets.destroy', $target->id) }}" method="POST" style="display:inline;">
                          @csrf
                          @method('DELETE')
                          <button type="submit" class="w-32-px h-32-px bg-danger-focus text-danger-main rounded-circle d-inline-flex align-items-center justify-content-center" onclick="return confirm('Are you sure you want to delete this target?')">
